...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.statusLog.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.status-logs.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="related_type">{{ trans('cruds.statusLog.fields.related_type') }}</label>
                <input class="form-control {{ $errors->has('related_type') ? 'is-invalid' : '' }}" type="text" name="related_type" id="related_type" value="{{ old('related_type', '') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('related') ? 'is-invalid' : '' }}" type="number" name="related" id="related" value="{{ old('related', '') }}" step="1">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('status_from') ? 'is-invalid' : '' }}" name="status_from_id" id="status_from_id">
                    @foreach($status_froms as $id => $entry)
                        <option value="{{ $id }}" {{ old('status_from_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('status_to') ? 'is-invalid' : '' }}" name="status_to_id" id="status_to_id">
                    @foreach($status_tos as $id => $entry)
                        <option value="{{ $id }}" {{ old('status_to_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('changed_by') ? 'is-invalid' : '' }}" name="changed_by_id" id="changed_by_id">
                    @foreach($changed_bies as $id => $entry)
                        <option value="{{ $id }}" {{ old('changed_by_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
