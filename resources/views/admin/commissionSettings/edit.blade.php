    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.commission-settings.update", [$commissionSetting->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('agent') ? 'is-invalid' : '' }}" name="agent_id" id="agent_id">
                    @foreach($agents as $id => $entry)
                        <option value="{{ $id }}" {{ (old('agent_id') ? old('agent_id') : $commissionSetting->agent->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('agent'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('university') ? 'is-invalid' : '' }}" name="university_id" id="university_id">
                    @foreach($universities as $id => $entry)
                        <option value="{{ $id }}" {{ (old('university_id') ? old('university_id') : $commissionSetting->university->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('curse') ? 'is-invalid' : '' }}" name="curse_id" id="curse_id">
                    @foreach($curses as $id => $entry)
                        <option value="{{ $id }}" {{ (old('curse_id') ? old('curse_id') : $commissionSetting->curse->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('curse'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('comission_commited') ? 'is-invalid' : '' }}" type="text" name="comission_commited" id="comission_commited" value="{{ old('comission_commited', $commissionSetting->comission_commited) }}">
