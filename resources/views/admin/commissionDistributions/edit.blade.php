...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.commissionDistribution.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.commission-distributions.update", [$commissionDistribution->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('student') ? 'is-invalid' : '' }}" name="student_id" id="student_id">
                    @foreach($students as $id => $entry)
                        <option value="{{ $id }}" {{ (old('student_id') ? old('student_id') : $commissionDistribution->student->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('commission_settings') ? 'is-invalid' : '' }}" name="commission_settings_id" id="commission_settings_id">
                    @foreach($commission_settings as $id => $entry)
                        <option value="{{ $id }}" {{ (old('commission_settings_id') ? old('commission_settings_id') : $commissionDistribution->commission_settings->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_commission_paid') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="is_commission_paid" id="is_commission_paid" value="1" {{ $commissionDistribution->is_commission_paid || old('is_commission_paid', 0) === 1 ? 'checked' : '' }}>
