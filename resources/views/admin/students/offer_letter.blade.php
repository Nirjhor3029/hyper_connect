    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.students.update", [$student->id]) }}" enctype="multipart/form-data">
                ...
                <input type="hidden" name="admission_stage" value="offer_processing">
                <input type="hidden" name="admission_status" value="offer_applied">
                ...
                <div class="form-group">
                    <label for="offer_letter_attachments">{{ trans('cruds.student.fields.offer_letter_attachments') }}</label>
                    <div class="needsclick dropzone {{ $errors->has('offer_letter_attachments') ? 'is-invalid' : '' }}" id="offer_letter_attachments-dropzone">
                    </div>
                    @if($errors->has('offer_letter_attachments'))
                        <div class="invalid-feedback">
                            {{ $errors->first('offer_letter_attachments') }}
                        </div>
                    @endif
                    <span class="help-block">{{ trans('cruds.student.fields.offer_letter_attachments_helper') }}</span>
                </div>
...
@section('scripts')
    ...
    <script>
        var uploadedOfferLetterAttachmentsMap = {}
        Dropzone.options.offerLetterAttachmentsDropzone = {
            url: '{{ route('admin.students.storeMedia') }}',
            maxFilesize: 10, // MB
            addRemoveLinks: true,
            headers: {
            ...
            success: function (file, response) {
                $('form').append('<input type="hidden" name="offer_letter_attachments[]" value="' + response.name + '">')
                uploadedOfferLetterAttachmentsMap[file.name] = response.name
            },
            removedfile: function (file) {
                file.previewElement.remove()
                var name = ''
                if (typeof file.file_name !== 'undefined') {
                    ...
                    name = uploadedOfferLetterAttachmentsMap[file.name]
                }
                $('form').find('input[name="offer_letter_attachments[]"][value="' + name + '"]').remove()
            },
            init: function () {
                @if(isset($student) && $student->offer_letter_attachments)
                var files =
                    {!! json_encode($student->offer_letter_attachments) !!}
                    for (var i in files) {
                    var file = files[i]
                    this.options.addedfile.call(this, file)
                    file.previewElement.classList.add('dz-complete')
                    $('form').append('<input type="hidden" name="offer_letter_attachments[]" value="' + file.file_name + '">')
                }
                @endif
            },
            error: function (file, response) {
