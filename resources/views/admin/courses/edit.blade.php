    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.courses.update", [$course->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('university') ? 'is-invalid' : '' }}" name="university_id" id="university_id" required>
                    @foreach($universities as $id => $entry)
                        <option value="{{ $id }}" {{ (old('university_id') ? old('university_id') : $course->university->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('program') ? 'is-invalid' : '' }}" name="program_id" id="program_id" required>
                    @foreach($programs as $id => $entry)
                        <option value="{{ $id }}" {{ (old('program_id') ? old('program_id') : $course->program->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Course::COURSE_MODE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('course_mode') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="course_mode_{{ $key }}" name="course_mode" value="{{ $key }}" {{ old('course_mode', $course->course_mode) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total_fees') ? 'is-invalid' : '' }}" type="number" name="total_fees" id="total_fees" value="{{ old('total_fees', $course->total_fees) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('no_of_semester') ? 'is-invalid' : '' }}" type="number" name="no_of_semester" id="no_of_semester" value="{{ old('no_of_semester', $course->no_of_semester) }}" step="1">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('course_details') ? 'is-invalid' : '' }}" name="course_details" id="course_details">{!! old('course_details', $course->course_details) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('scholarship') ? 'is-invalid' : '' }}" type="text" name="scholarship" id="scholarship" value="{{ old('scholarship', $course->scholarship) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('commission') ? 'is-invalid' : '' }}" type="number" name="commission" id="commission" value="{{ old('commission', $course->commission) }}" step="1">
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Course::ENGLISH_REQUIREMENT_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('english_requirement') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="english_requirement_{{ $key }}" name="english_requirement" value="{{ $key }}" {{ old('english_requirement', $course->english_requirement) === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('custom_message_for_student') ? 'is-invalid' : '' }}" name="custom_message_for_student" id="custom_message_for_student">{!! old('custom_message_for_student', $course->custom_message_for_student) !!}</textarea>
