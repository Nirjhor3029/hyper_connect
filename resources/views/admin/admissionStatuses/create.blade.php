...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.admissionStatus.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.admission-statuses.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('admission_stage') ? 'is-invalid' : '' }}" name="admission_stage_id" id="admission_stage_id">
                    @foreach($admission_stages as $id => $entry)
                        <option value="{{ $id }}" {{ old('admission_stage_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
